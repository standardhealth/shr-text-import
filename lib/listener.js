...
const {SHRParser} = require('./parsers/SHRParser');
const {Namespace, DataElement, Concept, Cardinality, Identifier, IdentifiableValue, RefValue, PrimitiveIdentifier, ChoiceValue, IncompleteValue, ValueSetConstraint, CodeConstraint, IncludesCodeConstraint, TypeConstraint, CardConstraint, TBD, PRIMITIVES} = require('shr-models');
...
class Importer extends SHRParserListener {
  ...
  processType(ctx, min, max) {
    if (ctx.elementWithConstraint()) {
      ...
      if (cst.elementTypeConstraint()) {
        ...
        const code = this.processFullyQualifiedCodeOrTBD(cst.elementCodeValueConstraint());
        ...
        const code = this.processFullyQualifiedCode(cst.elementIncludesCodeValueConstraint().fullyQualifiedCode());
  ...
  processFullyQualifiedCodeOrTBD(ctx) {
    if (ctx.fullyQualifiedCode()) {
      return this.processFullyQualifiedCode(ctx.fullyQualifiedCode());
    } else if (ctx.tbd() && ctx.tbd().STRING()) {
      return new Concept('urn:tbd', '');
    }

    return new Concept('urn:unknown', 'unknown');
  }
