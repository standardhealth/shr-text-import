...
const {SHRParser} = require('./parsers/SHRParser');
const {Namespace, DataElement, Concept, Cardinality, Identifier, IdentifiableValue, RefValue, PrimitiveIdentifier, ChoiceValue, IncompleteValue, ValueSetConstraint, CodeConstraint, IncludesCodeConstraint, BooleanConstraint, TypeConstraint, CardConstraint, TBD, PRIMITIVES} = require('shr-models');
...
class Importer extends SHRParserListener {
  ...
  enterDataDefsDoc(ctx) {
    ...
    if (ctx.descriptionProp()) {
      this._nsMap[ns].description = stripDelimitersFromToken(ctx.descriptionProp().STRING());
    }
  }
  ...
  processCodeOrFQCode(ctx) {
    ...
    if (ctx.fullyQualifiedCode()) {
      ...
      code = new Concept('urn:tbd', 'TBD');
      if (ctx.tbd() && ctx.tbd().STRING()) {
        code.display = stripDelimitersFromToken(ctx.tbd().STRING());
      }
    }
    return code;
  }
