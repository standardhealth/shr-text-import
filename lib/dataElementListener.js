...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  processElementWithConstraint(ctx) {
    ...
    if (ctx.simpleOrFQName() || ctx.specialWord()) {
      idText = ctx.simpleOrFQName() ? ctx.simpleOrFQName().getText() : ctx.specialWord().getText();
      ...
      //console.log("element bracket path");
      const ebp = ctx.elementBracketPath();
      const ebpFirst = ebp.elementBracketPathFirstPart();
      idText = ebpFirst.simpleOrFQName() ? ebpFirst.simpleOrFQName().getText() : ebpFirst.specialWord().getText();
    ...
    if (ctx.elementConstraint()) {
      ...
      if (cst.elementTypeConstraint()) {
        ...
        /*if (idText == 'Value') {
          path.push(new PrimitiveIdentifier('concept'));
        }*/
  ...
  enterField(ctx) {
    if (ctx.propertyField()) {
      ...
      if (this._currentDef.value && ctx.elementWithConstraint().getText().startsWith('Value')) {
        //console.log("current def and value");
        //console.log(this._currentDef);
        ...
        if (match) {
          ...
          //console.log(ctx.elementWithConstraint().getText());
          //console.log("current definition")
          //console.log(this._currentDef);
          /*console.log('field is added over here');
          console.log(field);
          console.log("field over here");
          console.log(this._currentDef.fields);
          console.log(ctx.elementWithConstraint().getText());
          console.log(typeof ctx.elementWithConstraint().getText());
          console.log(ctx.elementWithConstraint().getText().startsWith('Value'));*/
          this.addFieldToCurrentDef(field);
        }
      }
    }
