  ...
  processElementWithConstraint(ctx) {
    ...
    if (ctx.elementConstraint()) {
      ...
      if (cst.elementTypeConstraint()) {
        ...
        const minus = cst.elementIntegerConstraint().MINUS() ? '-' : '';
        const num = cst.elementIntegerConstraint().WHOLE_NUMBER().getText();
        const int = parseInt(minus + num);
        value.addConstraint(new FixedValueConstraint(int, 'integer', path));
      } else if (cst.elementDecimalConstraint()) {
        const minus = cst.elementDecimalConstraint().MINUS() ? '-' : '';
        const wholePart = cst.elementDecimalConstraint().WHOLE_NUMBER()[0].getText();
        const dot = cst.elementDecimalConstraint().DOT().getText();
        const decimalPart = cst.elementDecimalConstraint().WHOLE_NUMBER()[1] ? 
          cst.elementDecimalConstraint().WHOLE_NUMBER()[1].getText() : '';
        const decimal = parseFloat(minus + wholePart + dot + decimalPart);
