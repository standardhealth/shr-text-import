...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterElementWithConstraint(ctx) {
    ...
    if (ctx.simpleOrFQName() || ctx.specialWord()) {
      ...
      const idText = ebp.simpleOrFQName() ? ebp.simpleOrFQName().getText() : ebp.specialWord().getText();
      value = new IncompleteValue(this.resolveToIdentifier(idText));
      if (ebp.simpleName()) {
        path = ebp.simpleName().map(ctx => this.resolveToIdentifier(ctx.getText()));
      }
      if (ebp.primitive()) {
        path.push(new PrimitiveIdentifier(ebp.primitive().getText()));
      }
    }
  ...
  resolveBindingStrengthForVSConstraint(vsConstraint) {
    if (vsConstraint.bindingStrength()) {
      ...
      if (bindingCtx.KW_MUST_BE()) {
        return vsConstraint.KW_EXTENSIBLE() ? EXTENSIBLE : REQUIRED;
