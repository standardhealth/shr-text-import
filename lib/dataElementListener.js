...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterElementDef(ctx) {
    ...
    if (ctx.elementHeader().simpleName().LOWER_WORD()) { logger.error('Element name \'%s\' should begin with a capital letter. ERROR_CODE:1101', ctx.elementHeader().simpleName().getText()); }
  }
  ...
  enterEntryDef(ctx) {
    ...
    if (ctx.entryHeader().simpleName().LOWER_WORD()) { logger.error('Entry Element name \'%s\' should begin with a capital letter. ERROR_CODE:1102', ctx.entryHeader().simpleName().getText()); }
  }
  ...
  resolveValueSetForVSConstraint(vsConstraint) {
    ...
    if (vsConstraint.valueset().PATH_URL()) {
      ...
      if (!found) {
        logger.error('Unable to resolve value set reference: %s. ERROR_CODE:1103', name);
        vs = `urn:tbd:${name}`;
      }
  ...
  resolveBindingStrengthForVSConstraint(vsConstraint) {
    if (vsConstraint.bindingInfix()) {
      ...
      logger.error('Unsupported binding strength: %s.  Defaulting to REQUIRED. ERROR_CODE:1104', bindingCtx.getText());
    }
    return vsConstraint.KW_IF_COVERED() ? EXTENSIBLE : REQUIRED;
  }
