...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterDoc(ctx) {
    ...
    if (ctx.docHeader().namespace()) {
      ...
      logger.error('Namespace declaration not found. ERROR_CODE:11???');
    }
    this._currentNs = ns;
    ...
    if (docHeader.version()) {
      ...
      logger.error('Grammar declaration not found. ERROR_CODE 11???');
    }
  }
  ...
  enterField(ctx) {
    if (ctx.propertyField()) {
      ...
      if (this._currentDef.fields.some(f => f.identifier && f.identifier.equals(field.identifier))) {
        logger.error('Property "%s" already exists. ERROR_CODE:11???', field.identifier.name);
      }
      else {
      ...
      if (ctx.elementWithConstraint().getText().startsWith('Value')) {
        if (this._currentDef.value) {
          if (this._currentDef.value instanceof ChoiceValue) {
            ...
            if(!marked) {
              logger.error('Choice value constrained without specifying the specific choice. ERROR_CODE:11???')
            }
          }
        ...
        if (this._currentDef.fields.length > 0) {
          //console.log(field.identifier);
          //console.log(field.effectiveIdentifier);
          this._currentDef.fields.forEach(function(element) {
            //console.log(element.constraints);
            //console.log(element.identifier.name);
            //console.log(element.effectiveIdentifier.name);
          });
        }
  ...
  processCountAndTypesForValue(typeCtxArr) {
    ...
    let match = typeCtxArr[0].getText().match(/[0-9]+[..]/);
    if (match) {
      logger.error("Value should not be declaring cardinality. ERROR:11???");
    }
    else if (typeCtxArr[0].getText().length == 0) {
      logger.error("Missing a value element. ERROR:11???");
    }
    else {
  ...
  resolveValueSetForVSConstraint(vsConstraint) {
    ...
    if (vsConstraint.valueset().PATH_URL()) {
      ...
      let found = false;
      console.log(this._currentNs);
      console.log(this._usesNs);
      console.log(name);
      console.log(this._specs.valueSets);
