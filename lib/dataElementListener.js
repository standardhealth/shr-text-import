...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterDoc(ctx) {
    ...
    if ( isNullOrUndefined(ctx) || isNullOrUndefined(ctx.docHeader()) 
      || isNullOrUndefined(ctx.docHeader().namespace() ) || isNullOrUndefined(ctx.docHeader().namespace().getText() )) {
      logger.error('Missing or invalid  namespace. ERROR_CODE: 90003 ');
      return;  
    }
  ...
  enterField(ctx) {
    if (ctx.propertyField()) {
      if ( isNullOrUndefined(ctx.propertyField().count()) || isNullOrUndefined(ctx.propertyField().propertyFieldType())) {
        return;
      }
  ...
  processCountAndTypes(countCtx, typeCtxArr) {
    ...
    if (isNullOrUndefined( countCtx) || isNullOrUndefined( typeCtxArr)) {
      logger.error('Missing Cardinality.  ERROR_CODE: 90007');
      return(value);
    }
  ...
  getMinMax(countCtx) {
    ...
    if ( isNullOrUndefined( countCtx)) {
      logger.error('Invalid property.  No cardinality;  ERROR_CODE: 90004');
      return( [0,1] );
    }

    const cards = countCtx.WHOLE_NUMBER();
    if ( isNullOrUndefined(cards) ) {
      logger.error('Invalid property.  No cardinality;  ERROR_CODE: 90005');
      return( [0,1] );
    }
    if ( cards.length < 1  ) {
      logger.error('Invalid property.  No cardinality;  ERROR_CODE: 90006');
      return( [0,1] );
    }
