...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterDoc(ctx) {
    ...
    if ( isNullOrUndefined(ctx) || isNullOrUndefined(ctx.docHeader()) 
      || isNullOrUndefined(ctx.docHeader().namespace()  ))
      //ctx === undefined || ctx === null ||
      //ctx.docHeader() === undefined || ctx.docHeader() === null ||
      //ctx.docHeader().namespace() === undefined || ctx.docHeader().namespace() === null ) 
    {
      logger.debug('Missing Grammar name.  ERROR NUMBER 90002');
      return;
    }
  ...
  enterValue(ctx) {
    if (isNullOrUndefined (ctx) || isNullOrUndefined (ctx.valueType()) ) {
      logger.debug('Value is missing value element.  ERROR NUMBER 90007');
      return;
    }
    const value = this.processCountAndTypesForValue(ctx.valueType());
  ...
  enterElementWithConstraint(ctx) {
    let value;
    let path = [];
    let min, max;
    if (ctx.count()) {
    ...
    if (ctx.elementConstraint()) {
      ...
      if (cst.elementTypeConstraint()) {
        ...
        value.addConstraint(new CodeConstraint(code, [...path, new Identifier('shr.core', 'Units'), new Identifier('shr.core', 'concept')]));
  ...
  enterPropertyField(ctx) {
    const field = this.processCountAndTypes(ctx.count(), [ctx.propertyFieldType()]);
  ...
  processCountAndTypesForValue(typeCtxArr) {
    if ( isNullOrUndefined(typeCtxArr)) {
      logger.error('Value is missing value element. ERROR NUMBER 90009');
      return;
    }
    if (typeCtxArr.length > 1) {
      const value = new ChoiceValue();
    ...
    if (isNullOrUndefined(typeCtxArr[0])) {
      logger.error('Value is missing value element. ERROR NUMBER 90021');
      return;
    }
  ...
  processType(ctx, min, max) {
    let value;
    if ( isNullOrUndefined(ctx)  || isNullOrUndefined(min ) ||  isNullOrUndefined(max )
    ) {
      logger.error('Value is missing value element. ERROR NUMBER 90009');
      return;
    }
    /*
    if ( isNullOrUndefined(ctx.specialWord ) ) {
      logger.error('Value is missing value element. ERROR NUMBER 90011');
      return;
    }
    */
   /*
    if ( isNullOrUndefined(ctx.simpleOrFQName() ) 
    ) {
      logger.error('Value is missing value element. ERROR NUMBER 90010');
      return;
    }
    */
   ...
   //   value = this.enterElementWithConstraint(ctx.elementWithConstraint());
   // }
    else if (isNullOrUndefined(ctx.elementWithConstraint) && typeof ctx.elementWithConstraint === 'function' && ctx.elementWithConstraint()) {
      value = this.enterElementWithConstraint(ctx.elementWithConstraint());
    }
    ...
    */
    if ( value !== 'undefined' && value != null) {
      ...
      logger.error( 'Undefined value.  ERROR CODE 90016.');
    }
    return value;
  ...
  resolveValueSetForVSConstraint(vsConstraint) {
    ...
    //console.log('vsConstraint=' + vsConstraint);
    //console.log('vsConstraint.valueset()=' + vsConstraint.valueset());
    //console.log('vsConstraint.valueset().PATH_URL()=' + vsConstraint.valueset().PATH_URL());
    if ( isNullOrUndefined( vsConstraint ) 
    || isNullOrUndefined( vsConstraint.valueset() )
    //|| isNullOrUndefined( vsConstraint.valueset().PATH_URL() )
    || ! (typeof vsConstraint.valueset().PATH_URL === 'function')
    //|| isNullOrUndefined( vsConstraint.valueset().PATH_URL().getText())
    //|| isNullOrUndefined( vsConstraint.valueset().simpleName() ) 
    //|| isNullOrUndefined( vsConstraint.valueset().simpleName().getText() )
    //|| isNullOrUndefined( vsConstraint.valueset().getText() )
    ) {
      console.log('null vsConstraint.valueset().PATH_URL()');
      logger.error('Child inherits and defines a value.  ERROR CODE 90018');
      return( null);
    }
    let vs = vsConstraint.valueset().getText();
    if (isNullOrUndefined(vs) ) {
      logger.error('Child inherits and defines a value.  ERROR CODE 90019');
      return( null);
    }

    if (isNullOrUndefined(vsConstraint.valueset().PATH_URL )) {
      logger.error('Child inherits and defines a value.  ERROR CODE 90020');
      return( null);
    }
    if (vsConstraint.valueset().PATH_URL() !== null && typeof vsConstraint.valueset().PATH_URL() !== 'function' ) {
      logger.error('Child inherits and defines a value.  ERROR CODE 90022');
      return( null);
    }
    else {
      console.log('vsConstraint.valueset().PATH_URL=' + vsConstraint.valueset().PATH_URL);
      let temp2 = vsConstraint.valueset().PATH_URL();
  ...
  getMinMax(countCtx) {
    ...
    if (cards === undefined || cards === null || cards.length < 2) {
      logger.error('Invalid property.  No cardinality;  ERROR NUMBER 90003');
      return[0,1];
    }
