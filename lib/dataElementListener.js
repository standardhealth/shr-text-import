...
class DataElementImporter extends SHRDataElementParserListener {
    ...
    if (identifier.isSpecialKeyWord) {
      ...
      if (this._currentDef.isEntry) {
        ...
        if (parentInfo.type !== 'entry' && parentInfo.type !== 'abstract') {
          logger.error('Entry %s cannot declare %s as its parent since %s is not an Entry or Abstract.  ERROR_CODE:11XXX', this._currentDef.identifier.name, identifier.name, identifier.name);
        }
      } else if (this._currentDef.isAbstract) {
        // Abstracts can only inherit from another Abstract
        if (parentInfo.type !== 'abstract') {
          logger.error('Abstract %s cannot declare %s as its parent since %s is not an Abstract.  ERROR_CODE:11XXX', this._currentDef.identifier.name, identifier.name, identifier.name);
        }
      } else if (this._currentDef.isGroup) {
        // Groups can only inherit from another Group
        if (parentInfo.type !== 'group') {
          logger.error('Group %s cannot declare %s as its parent since %s is not a Group.  ERROR_CODE:11XXX', this._currentDef.identifier.name, identifier.name, identifier.name);
        }
      } else /* is an element */ {
        // Elements can only inherit from another Element
        if (parentInfo.type !== 'element') {
          logger.error('Element %s cannot declare %s as its parent since %s is not an Element.  ERROR_CODE:11XXX', this._currentDef.identifier.name, identifier.name, identifier.name);
        }
      }
  ...
  enterValue(ctx) {
    ...
    if (this._currentDef.isGroup || this._currentDef.isEntry || this._currentDef.isAbstract) {
      logger.error('Cannot declare Value on a non-Element.  ERROR_CODE:11XXX');
    } else if (ctx.COLON() && this._currentDef.basedOn.length > 0) {
      logger.error('Cannot redeclare Value on Element that has a parent.  Constrain Value instead.  ERROR_CODE:11XXX');
    }
  ...
  enterField(ctx) {
    ...
    if (ctx.propertyField()) {
      ...
      if (isElement) {
        logger.error('Cannot declare properties on an Element since Elements do not have properties.  ERROR_CODE:11XXX');
      }
      ...
      if (/^Value([.\[\s].*)?$/.test(ctx.elementWithConstraint().getText())) {
        ...
        // to make any changes in the grammar.  Consider changing in 6.1.
        if (!isElement) {
          logger.error('Cannot constrain Value on a non-Element since non-Elements do not have Values.  ERROR_CODE:11XXX');
        }
        ...
        // to make any changes in the grammar.  Consider changing in 6.1.
        if (isElement) {
          logger.error('Cannot constrain properties on Elements since Elements only have Values.  ERROR_CODE:11XXX');
        }
