...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterValue(ctx) {
    // Check to ensure it does not have a parent (in which case it cannot redeclare Value)
    // NOTE: This relies on the current grammar requiring parents be declared before values
    // Check for ctx.COLON() because the grammar re-uses this rule for the KW_ONLY constraint too.
    if (ctx.COLON() && this._currentDef.basedOn.length > 0) {
