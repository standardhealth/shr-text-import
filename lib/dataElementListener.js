    ...
    if (ctx.elementWithConstraint()) {
      ...
      if (ewc.elementConstraint()) {
        ...
        if (cst.elementTypeConstraint()) {
          // TODO: Support type constraints that have choices!  (i.e., remove the [0] indexer below and do something else)
          const newIdentifier = this.resolveToIdentifierOrTBD(cst.elementTypeConstraint().typeConstraintTarget()[0]);
          ...
          for (const typeConstraint of cst.elementIncludesTypeConstraint().includesTypeConstraintTarget()) {
            const newIdentifier = this.resolveToIdentifierOrTBD(typeConstraint.typeConstraintTarget());
            if (newIdentifier.isValueKeyWord) {
              logger.error('Fields cannot be constrained to type "Value". ERROR_CODE:11025');
            } else {
