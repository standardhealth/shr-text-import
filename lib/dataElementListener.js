...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterField(ctx) {
    if (ctx.propertyField()) {
      ...
      ctx.elementWithConstraint()
      if (this._currentDef.value && ctx.elementWithConstraint().getText().startsWith('Value')) {
        field.constraints.forEach(c => this._currentDef.value.addConstraint(c));
      }
      else {
        const match = this._currentDef.fields.find(f => f.identifier && f.identifier.equals(field.identifier));
        if (match) {
