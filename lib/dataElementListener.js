...
class DataElementImporter extends SHRDataElementParserListener {
  ...
  enterField(ctx) {
    if (ctx.propertyField()) {
      ...
      //console.log(field);
      //console.log('299 line');
      if (this._currentDef.value && field.identifier.equals(this._currentDef.value.identifier)) {
        if (field.card && !field.card.equals(this._currentDef.value.card)) {
          const cardConstraint = new CardConstraint(field.card);
          this._currentDef.value.addConstraint(cardConstraint);
        }
