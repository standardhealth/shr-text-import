...
function shorthandFromCodesystem(cs) {
  const shorthands = {
    ...
    "http://hl7.org/fhir/ValueSet/observation-category": "OBSCAT",
...
class DataElementFormatter {
  ...
  formatDataElementOutput(de) {
    ...
    if (de.value) {
      value = this.formatDataElementValue(de);
      outputs.push(value);
    }
    ...
    if ('fields' in de && de.fields.length > 0) {
      ...
      for (const f of de.fields) {
        if (f.inheritance != 'inherited') {
          fields.push(this.formatValue(f));
        }
      }
  ...
  formatDeclaration(de) {
    ...
    const formattedTitle = `${formattedModifier}Element:\t\t${de.identifier.name}`
  ...
  formatDataElementValue(de) {
    return `Value:\t\t\t${this.formatValue(de.value)}`;
  }

  formatValue(value) {
    let formattedValue;

    formatByValueType(value);
    if (!formattedValue) return;

    if ('constraints' in value && value.constraints.length > 0) {
      for (const con of value.constraints) {
        formattedValue = formatConstraintOnFormattedValue(formattedValue, con);
      }
    }
    ...
    function formatByValueType(value) {
      switch (value.constructor.name) {
        case 'IdentifiableValue':
          ...
          break;
        case 'RefValue':
          formattedValue = `ref(${value.identifier.name})`;
          break;
        case 'ChoiceValue':
          // formattedValue = value.identifier.name;
          return ''
    ...
    function formatConstraintOnFormattedValue(value, constraint) {
      switch (constraint.constructor.name) {
        case 'TypeConstraint':
          return formatByTypeConstraint(value, constraint);
          break;
        case 'ValueSetConstraint':
          return formatByValueSetConstraint(value, constraint);
          break;
        case 'CardConstraint':
          return formatByCardConstraint(value, constraint);
          break;
        // case 'FixedValueConstraint':
        // formatByFixedValueConstraint(value);
        // break;
        case 'IncludesTypeConstraint':
          return formatByIncludesTypeConstraint(value, constraint);
          break;
        case 'IncludesCodeConstraint':
          return formatByIncludesCodeConstraint(value, constraint);
          break;
        case 'BooleanConstraint':
          return formatByBooleanConstraint(value, constraint);
          break;
        case 'CodeConstraint':
          return formatByCodeConstraint(value, constraint);
          break;
        default:
    ...
    function formatByTypeConstraint(value, constraint) {
    ...
    function formatByValueSetConstraint(value, constraint) {
      ...
      switch (constraint.bindingStrength) {
        case 'REQUIRED':
          return `${value} must be from ${vs}`
          break;
        case 'EXTENSIBLE':
          return `${value} must be from ${vs} if covered`
          break;
        case 'PREFERRED':
    ...
    function formatByCardConstraint(value, constraint) {
      let formattedConstraint = `${constraint.card.toString()} ${value}`;
      return formattedConstraint;
      // return value;
    }

    function formatByBooleanConstraint(value, constraint) {
      return `${value} is ${constraint.value}`;
    }

    function formatByCodeConstraint(value, constraint) {
      let formattedCode = `${shorthandFromCodesystem(constraint.code.system)}#${constraint.code.code}`
      return `${value} is ${formattedCode}`;
    }

    function formatByIncludesTypeConstraint(value, constraint) {
      let formattedConstraint = `\tincludes ${constraint.card.toString()} ${constraint.isA.name}`;
      return [value, formattedConstraint].join('\n');
    }

    function formatByIncludesCodeConstraint(value, constraint) {
