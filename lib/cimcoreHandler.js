  ...
  constructValue(value, de) {
    ...
    if ('card' in value) {
      ...
      if ('history' in value.card) {
        ...
        const currentHistory = value.card.history.find(h => h.source == de.fqn);
        if (currentHistory) {
          constructedValue.addConstraint(this.constructCardHistoryConstraints(currentHistory));
        }
      }
    }
  ...
  constructConstraints(constraints) {
    ...
    function constructFixedValueConstraint(constraint, path) {
      if (constraint.type == 'code') {
        const fixedValueConstraint = new CodeConstraint(constraint.value, path);
...
class ValueSetConstructor {
  ...
  constructValues(values) {
    ...
    for (const value of values) {
      // constructCode(value.code, value.system, value.description);
      let code = {
        "system": value.system,
        "code": value.code,
...
class DataElementFormatter {
  ...
  formatDataElementOutput(de) {
    ...
    if (de.value) {
      if (de.value.inheritance != 'inherited') {
    ...
    if ('fields' in de && de.fields.length > 0) {
      ...
      for (const f of de.fields) {
        if (f.inheritance != 'inherited') {
          fields.push(this.formatValue(f, de, true));
        }
      }
    ...
    // fields = this.formatFields(de);
    // if (fields) outputs.push(fields);


    // title = '{0:20}{1}'.format(title_text, de.label)
    // concept = de.concepts
    // based_on = de.based_on
    // description = de.build_description()
    // # TODO finish value
    // value = de.value
    // properties = '\n'.join(de.properties)
    // definitions = de.build_definitions(elements)
  ...
  formatValue(value, de, isField=false) {
    let formattedValue;

    formattedValue = formatByValueType(value);

    if (!isField) return `Value:\t\t\t${formattedValue}`; 

    return formattedValue;
  
    function formatByValueType(value) {
      let formattedByType;
      switch (value.constructor.name) {
        case 'IdentifiableValue':
          formattedByType = value.identifier.name;
          break;
        case 'RefValue':
          formattedByType = `ref(${value.identifier.name})`;
          break;
        case 'ChoiceValue':
          // formattedValue = value.identifier.name;
          let choiceStrings = [];
          for (const opt of value.options) {
            choiceStrings.push(formatByValueType(opt));
          }
          return choiceStrings.join(' or ')         
          // return ''
          break;
        case 'TBD':
          formattedByType = `TBD "${value.text}`;
          break;
        default:
          break;
      }

      if (!formattedByType) return;
        ...
        for (const con of value.constraints) {
          if (con.lastModifiedBy.fqn == de.identifier.fqn) {
            if (con.path.length > 0) {
              formattedByType = formatWithPath(formattedByType, con.path);
            }
            formattedByType = formatWithConstraint(formattedByType, con);
          }
        }
      }

      if (!value.card.equals(value.effectiveCard)) {
    ...
    function formatWithPath(formattedValue, paths) {
      let pathsClone = Object.assign({}, paths);
    ...
    function formatWithConstraint(value, constraint) {
      switch (constraint.constructor.name) {
        case 'TypeConstraint':
          return formatWithTypeConstraint(value, constraint);
          break;
        case 'ValueSetConstraint':
          return formatWithValueSetConstraint(value, constraint);
          break;
        case 'CardConstraint':
        //   return formatWithCardConstraint(value, constraint);
          return value;  
          break;
        // case 'FixedValueConstraint':
        // formatWithFixedValueConstraint(value);
        // break;
        case 'IncludesTypeConstraint':
          return formatWithIncludesTypeConstraint(value, constraint);
          break;
        case 'IncludesCodeConstraint':
          return formatWithIncludesCodeConstraint(value, constraint);
          break;
        case 'BooleanConstraint':
          return formatWithBooleanConstraint(value, constraint);
          break;
        case 'CodeConstraint':
          return formatWithCodeConstraint(value, constraint);
          break;
        default:
    ...
    function formatWithTypeConstraint(value, constraint) {
      if (constraint.onValue) {
        return `${value} value is type ${constraint.isA.name}`;
      } else {
        return `${value} is type ${constraint.isA.name}`;
      }
    }

    function formatWithValueSetConstraint(value, constraint) {
      ...
      switch (constraint.bindingStrength) {
        case 'REQUIRED':
          return `${value} from ${vs}`
          break;
        case 'EXTENSIBLE':
          return `${value} from ${vs} if covered`
          break;
        case 'PREFERRED':
          return `${value} should be from ${vs}`
          break;
        case 'EXAMPLE':
          return `${value} could be from ${vs}`
          break;
        default:
        return value  
          break;
    ...
    function formatWithCardConstraint(value, constraint) {
      let formattedConstraint = `${constraint.card.toString()} ${value}`;
      return formattedConstraint;
      // return value;
    }

    function formatWithBooleanConstraint(value, constraint) {
      return `${value} is ${constraint.value}`;
    }

    function formatWithCodeConstraint(value, constraint) {
      ...
      if (constraint.path.length >= 2 && constraint.path[0].name == 'Units' && constraint.path[1].name == 'Coding') {
        return `${value} with units ${formattedCode}`
      }

      return `${value} is ${formattedCode}`;
    }

    function formatWithIncludesTypeConstraint(value, constraint) {
      let formattedConstraint = `\tincludes ${constraint.card.toString()} ${constraint.isA.name}`;
      return [value, formattedConstraint].join('\n');
    }

    function formatWithIncludesCodeConstraint(value, constraint) {
      let shorthand = shorthandFromCodesystem(constraint.code.system)
      if (!shorthand) {
        shorthand = '';
      }
