...
class CimplExporter {
  ...
  constructor(namespaces, elements, valuesets, mappings) {
    this.headerFormatter = new FileHeaderFormatter();
  ...
  exportNamespaceDataElementFileToPath(namespace, path) {
    const nsDataElements = this.elements.filter(de => de.identifier.namespace == namespace);
    const header = this.headerFormatter.formatHeader('DataElement', namespace, [], [], []);
  }

  exportNamespaceValueSetFileToPath(namespace, path) {
    const nsValueSets = this.valuesets.filter(vs => vs.identifier.namespace == namespace);
  }

  exportNamespaceMappingFileToPath(namespace, path) {
    const nsMaps = this.mappings.filter(map => map.identifier.namespace == namespace);
  }

  
}

class NamespaceDataElementFileFormatter {
  constructor(ns) {
    this._uses = [];
    this._paths = [];
    this._codesystems = [];
...
// CodeSystem:     UCUM = http://unitsofmeasure.org 
  ...
  formatCodeSystem(cs) {
    return `CodeSystem:\t${cs} = ${shorthandFromCodesystem(cs)}`
  }

  formatPath(path) {
    return `Path:\t\t\t${path.shorthand} = ${path.url}`
  }
}
