...
class Preprocessor extends SHRDataElementParserVisitor {
  ...
  preprocessConfig(defaultsFile, file) {
    ...
    if (file != null) {
      try { configFile = JSON.parse(new FileStream(file)); } 
        ...
        logger.error("Invalid config file. Should be valid JSON dictionary. ERROR_CODE:1106");
        return defaults;
      }
    } else {
      logger.warn(`No project configuration file found, currently using default EXAMPLE identifiers. Auto-generating a proper 'config.json' in your specifications folder. ERROR_CODE:0101`);
      return defaults;
    }
    ...
    for (var key in defaults) {
      if (configFile[key] == null) {
        ...
        logger.warn("Configuration file missing key: %s, using default key: %s instead. ERROR_CODE:0102", key, defaults[key]);
      } else {
  ...
  checkVersion(version) {
    ...
    if (GRAMMAR_VERSION.major != major || GRAMMAR_VERSION.minor < minor) {
      logger.error('Unsupported grammar version: %s.%s. ERROR_CODE:1107', major, minor);
      return false;
    }
    return true;
  }
...
class PreprocessedData {
  ...
  resolvePath(name, ...namespace) {
    ...
    if (namespace.length == 0) {
      return { error: `Cannot resolve path without namespaces. ERROR_CODE:1117` };
    }
    ...
    if (!result.hasOwnProperty('url')) {
      result['error'] = `Failed to resolve path for ${name}. ERROR_CODE:1118`;
    } else if (conflict) {
      result['error'] = `Found conflicting path for ${name} in multiple namespaces: ${foundNamespaces}. ERROR_CODE:1119`;
    }
    return result;
  }
  ...
  resolveVocabulary(name, ...namespace) {
    ...
    if (!result.hasOwnProperty('url')) {
      result['error'] = `Failed to resolve vocabulary for ${name}. ERROR_CODE:1120`;
    } else if (conflict) {
      result['error'] = `Found conflicting vocabularies for ${name} in multiple namespaces: ${foundNamespaces}. ERROR_CODE:1121`;
    }
    return result;
  }
  ...
  resolveDefinition(name, ...namespace) {
    ...
    if (!result.hasOwnProperty('namespace')) {
      result['error'] = `Failed to resolve definition for ${name}. ERROR_CODE:1113`;
    } else if (foundNamespaces.length > 1) {
      result['error'] = `Found conflicting definitions for ${name} in multiple namespaces: ${foundNamespaces}. ERROR_CODE:1122`;
    }
    return result;
  }
}
