...
class Preprocessor extends SHRDataElementParserVisitor {
  ...
  visitDoc(ctx) {
    ...
    || isNullOrUndefined(ctx.docHeader().namespace()) || isNullOrUndefined(ctx.docHeader().namespace().getText() )) {
      logger.error('Missing namespace declaration. ERROR_CODE: 90008');
      return;
    }
  ...
  checkVersion(version) {
    if (isNullOrUndefined(version) ) {
      logger.error('Need Major & Minor version number for Grammar; %s.%s. ERROR_CODE: 90001');
      return(false);
    }
    ...
    if (isNullOrUndefined(major) || isNullOrUndefined(minor) ) {
      logger.error('Need Major & Minor version number for Grammar; %s.%s. ERROR_CODE: 90002');
      return(false);
    }
