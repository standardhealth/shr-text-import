...
class Preprocessor extends SHRDataElementParserVisitor {
  ...
  preprocessConfig(file) {
    //Abhijay TODO: Make this an antlr file
    ...
    if (file != null) {
      ...
      catch (e) { logger.error("Invalid config file. Should be valid JSON dictionary"); }
    } else {
      logger.warn("No project configuration file found, currently using default EXAMPLE identifiers. Create a proper 'config.txt' in your specifications folder");
      return defaults;
    }
...
class PreprocessedData {
  ...
  resolvePath(name, ...namespace) {
    ...
    if (name == 'default') {
      ...
      //TODO: Ask Chris if this should be replaced by config
    }
