...
//   }
  ...
  exportDataElementFileToNamespace(file, namespace, filePath) {
    ...
    const hierarchyPath = `${filePath}/${ns}.rtf`;
  ...
  exportValueSetFileToNamespace(file, namespace, filePath) {
    ...
    const hierarchyPath = `${filePath}/${ns}_vs.rtf`;
    mkdirp.sync(hierarchyPath.substring(0, hierarchyPath.lastIndexOf('/')));
    fs.writeFileSync(hierarchyPath, file);
  }
  ...
  exportMappingsFileToNamespace(file, namespace, filePath) {
    ...
    const hierarchyPath = `${filePath}/${ns}_map.rtf`;
    mkdirp.sync(hierarchyPath.substring(0, hierarchyPath.lastIndexOf('/')));
    fs.writeFileSync(hierarchyPath, file);
  }
}
...
class NamespaceFormatter {
  ...
  formatHeader(fileType) {
    switch (fileType) {
      case 'DataElement': {
        // return this.headerFormatter.formatDataElementFileHeader(fileType, this.ns, this._uses, this.dataElementFormatter.codesystems, this.dataElementFormatter.paths);
        break;
      }
      case 'ValueSet': {
...
class ValueSetFormatter {
  ...
  formatConcepts(vs) {
    ...
    for (const concept of vs.concepts) {
      const formattedConcept = `${rightPad('Concept:')}${this.formattedCodeFromCncpt(concept)}`;
  ...
  formatRuleByType(rule) {
    switch (rule.constructor.name) {
      ...
      case 'CardinalityMappingRule': {
        return `constrain ${rule.target.replace(/[ \t]+$/,'')} to ${rule.value}`;
      }
