...
function rightPad(text) {
  const numTabs = Math.min(4, 4 - Math.floor(text.length / 4));
  ...
  constructRules(rules) {
    ...
    function constructExcludesDescendentsRules(rules) {
      for (const rule of rules) {
        ...
        let excludesDescendantsRule = new ValueSetExcludesDescendentsRule(code);
...
class FileHeaderFormatter {
  ...
  formatHeader(fileType, ns, uses = [], codesystems = [], paths = []) {
    // const formattedType = this.formatFileType(fileType);
    ...
    return [
      ...
      `${rightPad('Description:')}:\t${ns.description}`,
      formattedUses,
      ...formattedPaths,
      ...formattedCS
    ].filter(l => l).join('\n');
...
class ValueSetFormatter {
  ...
  formatValueSet(valueSet) {
    ...
    if (valueSet.rules.length > 0) {
      rules = this.formatRules(valueSet.rules);
      outputs.push(rules);
    }
  ...
  formatRules(rules) {
    ...
    for (const rule of rules) {
      const formattedRule = this.formatRulesByType(rule);
      formattedRules.push(formattedRule);
    }
    return formattedRules;
  }

  formatRulesByType(rule) {
    const formattedRules = [];
    switch (rule.constructor.name) {
    case 'ValueSetIncludesDescendentsRule':
      formattedRules.push(this.formatIncludesDescendantsRule(rule));
      break;
    case 'ValueSetIncludesFromCodeRule':
      formattedRules.push(this.formatIncludesFromCodeRule(rule));
      break;
    case 'ValueSetIncludesFromCodeSystemRule':
      formattedRules.push(this.formatIncludesFromCodeSystemRule(rule));
      break;
    case 'ValueSetExcludesDescendentsRule':
      formattedRules.push(this.formatExcludesDescendentsRule(rule));
      break;
    default:
