...
function rightPad(text) {
  const numTabs = Math.max(1, 4 - Math.floor(text.length / 4));
...
class CimplExporter {
  constructor(specs) {
    ...
    this._mappings = [...specs.maps._targetMap][0][1].all;
  }
  ...
  exportNamespaceToPath(ns, path) {
    ...
    const nsMappings = this.mappings.filter(map => map.identifier.namespace == ns); // alternative too complex
    ...
    if (nsDataElements.length > 0) {
      ...
      // this.exportDataElementFilesToNamespace(formattedDataElementFile, ns, path);
    }
    ...
    if (nsMappings.length > 0) {
      const formattedMappingsFile = nsFormatter.formatMappingsFile();
      this.exportMappingsFileToNamespace(formattedMappingsFile, ns, path);
    }
  }

  exportDataElementFilesToNamespace(file, namespace, filePath) {
  ...
  exportValueSetFilesToNamespace(file, namespace, filePath) {
    let ns = namespace.replace(/\./, '-');
  ...
  exportMappingsFilesToNamespace(file, namespace, filePath) {
    let ns = namespace.replace(/\./, '-');
...
class NamespaceFormatter {
  ...
  formatHeader(fileType) {
    const fileHeader = this.headerFormatter.formatHeader(fileType, this.ns, this._uses, this._codesystems, this._paths);
    return fileHeader;
  }
  ...
  formatDataElementFile() {
    ...
    for (const de of this.nsElements) {
  ...
  formatMappingsFile() {
    const fileHeader = this.formatHeader('Mapping');

    const formattedMappings = [];
    for (const map of this.nsMappings) {
...
class FileHeaderFormatter {
  ...
  formatHeader(fileType, ns, uses = [], codesystems = [], paths = []) {
    const formattedFileDeclaration = this.formatFileDeclaration(fileType, ns);
    const formattedUses = (uses.length > 0) ? this.formatUses(uses) : null;
  ...
  formatFileDeclaration(fileType, ns) {
    const formattedDeclaration = [
      `${rightPad('Grammar:')}${fileType} 5.0`,
      `${rightPad('Namespace:')}${ns.namespace}`,
    ];

    if (['DataElement', 'ValueSet'].includes(fileType)) {
      if (ns.description) {
        formattedDeclaration.push(`${rightPad('Description:')}:${ns.description}`);
      }
    } else if (fileType == 'Mapping') {
      formattedDeclaration.push(`${rightPad('Target:')}:${'FIGURE THIS OUT'}`);
    }
  ...
  formatFileType(fileType) {
    return `${rightPad('Grammar:')}${fileType} 5.0`;
  }
  ...
  formatCodeSystem(cs) {
    return `${rightPad('CodeSystem:')}${cs} = ${shorthandFromCodesystem(cs)}`;
  }
...
class ValueSetFormatter {
  ...
  formatConcepts(vs) {
    ...
    for (const concept of vs.concepts) {
      const formattedConcept = `${rightPad('Concept:')}${formattedCodeFromConcept(concept)}`;
  ...
  formatIncludesCodeRule(rule) {
    const formattedCode = `${shorthandFromCodesystem(rule.code.system)}#${rule.code.code}`;
    return `${rightPad(formattedCode)}"${rule.code.display}"`;
  }

  formatIncludesFromCodeRule(rule) {
    const formattedCode = formattedCodeFromConcept(rule.code);
    return `Includes codes from ${formattedCode}`;
  }

  formatIncludesFromCodeSystemRule(rule) {
    const formattedCodeSystem = shorthandFromCodesystem(rule.system);
  ...
  formatExcludesDescendentsRule(rule) {
    const formattedCode = formattedCodeFromConcept(rule.code);
    return `Includes codes descending from ${formattedCodeFromConcept(formattedCode)}`;
  }
}
...
class MappingFormatter {
  ...
  formatMappingOutput(map) {
    const elMapping = this.formatElementMapping(map);
    // const fieldMappings
    return [
      elMapping
    ].join('\n');
  ...
  formatElementMapping(map) {
    return `${map.name} maps to ${map.targetItem}:`;
  }

  formatMappingsByType(map) {

    // for (type in map.mappings) {

    // }
  }

  formatFieldMapping(mappings) {
