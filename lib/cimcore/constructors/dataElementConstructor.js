...
class DataElementConstructor {
  ...
  constructValue(value, de) {
    ...
    default:
      logger.error('Unable to import property %s, unknown value type: %s ERROR_CODE:TBD', de.fqn, value.valueType);
      return;
    }
    ...
    //   value.constraintHistory = this.constructConstraintHistory(value);
    // }
  ...
  constructByType(constraint, cType, path, aggregate) {
    ...
    case 'subpaths':
      ...
      break;
    default:
      logger.error('Unable to import unknown constraint type: %s ERROR_CODE:TBD', cType);
      break;
    }

    if (aggregate && constructedConstraint) aggregate.push(constructedConstraint);
  ...
  constructFixedValueConstraint(constraint, path) {
    if (constraint.type == 'code') {
      ...
      logger.error('Unable to import FixedValueConstraint, unknown fixed value type: %s ERROR_CODE:TBD', constraint.type);
    }
  }
  ...
  constructConstraintHistory(value) {
    ...
    for (const cType in value.constraintHistory) {
      value.constraintHistory[cType].forEach(historyItem => {
        constructedConstraintHistory.add(this.constructByType(historyItem.constraint, cType, []), historyItem.source);
      });
    }
