...
class ValueSetConstructor {
  ...
  add(vsJSON) {
    ...
    if (constructedVS.rulesFilter.includesCode.rules.some(rule => rule.code.system.startsWith(this.config.projectURL))) {
      ...
      constructedVS.rulesFilter.includesCode.rules.forEach(rule => constructedCS.addCode(rule.code));
  ...
  constructRules(rules) {
    ...
    function constructByType(rules, rType) {
      ...
      case 'excludesDescendants':
        ...
        break;
      default:
        logger.error('Unable to import VS rule, unknown rule type: %s ERROR_CODE:TBD', rType);
        break;
      }
    }
  ...
  constructCodeSystem(constructedVS) {
    let currentPath = constructedVS.url.replace(/#[A-Za-z0-9]*VS/, '');
    let csPath;
    if (currentPath.indexOf('/vs/') >= 0) {
      csPath = currentPath.replace('/vs/', '/cs/');
    } else if (currentPath.indexOf('/ValueSet/') >= 0) {
      csPath = currentPath.replace('/ValueSet/', '/CodeSystem/');
    } else {
      csPath = `${currentPath}cs/`;
    }
