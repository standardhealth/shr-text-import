...
class ValueSetImporter extends SHRValueSetParserListener {
  ...
  pushCurrentDefinition() {
    ...
    if (this._specs.valueSets._nsMap.has(this._currentNs)) {
      if (this._specs.valueSets._nsMap.get(this._currentNs).has(this._currentDef.identifier._name)) {
        logger.error('ValueSet name "%s" already exists. ERROR_CODE:11034', this._currentDef.identifier._name);
      };
    };
