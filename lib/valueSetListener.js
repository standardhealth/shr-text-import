...
class ValueSetImporter extends SHRValueSetParserListener {
  ...
  enterValuesetDef(ctx) {
    ...
    if (h.URL()) {
      logger.error('Defining value sets by URL has been deprecated in ValueSet files.  ValueSet %s ignored. ERROR_CODE:1108', h.URL().getText());
      ...
      logger.error('Defining value sets by URN has been deprecated in ValueSet files.  ValueSet %s ignored. ERROR_CODE:1109', h.URN_OID().getText());
      // Set a dummy unsupported def so the rest of the parsing can occur -- but it won't be added to the definitions
      this._currentDef = new ValueSet(new Identifier('unsupported', 'Unsupported'), 'urn:unsupported');
      return;
    }
  ...
  enterValuesetFromCodeSystem(ctx) {
    ...
    if (typeof system === 'undefined') {
      logger.error('Couldn\'t resolve code system for alias: %s. ERROR_CODE:1110', alias);
      return;
    }
  ...
  enterUsesStatement(ctx) {
    logger.error('Uses statements have been deprecated in ValueSet files.  Uses statement ignored. ERROR_CODE:1111');
  }

  enterPathDef(ctx) {
    logger.error('Only default path definitions are allowed in ValueSet files.  Path definition ignored. ERROR_CODE:1112');
  }
