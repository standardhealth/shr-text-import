...
const {Specifications, Version, NamespaceContentProfile, ContentProfile, ContentProfileRule, Identifier, IdentifiableValue, ChoiceValue, PrimitiveIdentifier, PRIMITIVES} = require('shr-models');
...
class ContentProfileImporter extends SHRContentProfileParserListener {
  ...
  enterNamespaceFlag(ctx) {
    ...
    for (const de of this._specs.dataElements.all) {
      if (de.identifier.namespace === this._currentNs && (de.isEntry || de.isGroup)) {
        const pathStr = de.identifier.name;
