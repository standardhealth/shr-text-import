...
const {CimcoreImporter, CimplExporter, DataElementFormatter} = require('./cimcore/cimcoreHandler');
...
function importCIMCOREFromFilePath(filePath, expSpecs) {
  ...
  handler.readFiles(filePath + '/../../shr-cli/out/cimcore_old', expSpecs);
  ...
  // n = n.filter(ns=>[''].indexOf(ns.namespace) == -1);
  for (const ns of n.splice(0,26)) {
  ...
  //   let outDef = (new DataElementFormatter()).formatDataElement(element) || [];

  //   let sim = compareSimilarity(origDef, outDef.split('\n'));
  //   if (sim[1] != 100) {
  //     console.log('------------------------------------------------------');
  //     let a = false;
  //     // a = true;
