...
function importFromFilePath(filePath, configuration=[], specifications = new Specifications()) {
  ...
  for (const file of filesByType.contentProfile) {
    if (path.basename(file) === configuration.contentProfile) {
      contentProfileFound = true;
      contentProfileImporter.importFile(file);
    }
  }
  if (configuration.contentProfile && !contentProfileFound) {
    logger.error('Could not find content profile file: %s. ERROR_CODE:?????', configuration.contentProfile);
  }
