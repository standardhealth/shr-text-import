...
function importCIMCOREFromFilePath(filePath, expSpecs) {
  ...
  function compareSimilarity(orig, out) {
    ...
    strippedOrig = strippedOrig.filter(ln => ln !== 'Concept:TBD')
    strippedOrig = strippedOrig.filter(ln => ln !== '')
    let strippedOut = [];
    out.map(ln => ln.split('\n')).forEach(a => strippedOut.push(...a))
    strippedOut = strippedOut.map(ln => ln.replace(/\s/g, ''));
    ...
    return [`${per}% similar, ${totalLines - similarLines} dissimilar lines`, per]
  }
