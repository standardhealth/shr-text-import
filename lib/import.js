...
function importConfigFromFilePath(filePath) {
  ...
  configuration = (filesByType.config.length > 0) ? preprocessor.preprocessConfig(filesByType.config[0]) : preprocessor.preprocessConfig();
  ...
  detectType(file) {
    ...
    for (const l of lines) {
      ...
      if (match != null && match.length >= 2) {
        return match[1];
      } else if (file.endsWith('config.txt')) {
        return "Config"
      }
    }
  }
